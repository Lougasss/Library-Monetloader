...
function JNI:getAndroidID()
    local settingsSecure = self:findClass("android/provider/Settings$Secure")
    JNI:checkForJNIException(true)
    if not settingsSecure then
        return nil
    end

    local contentResolver = self:callObjectMethod(getActivity(), "getContentResolver", "()Landroid/content/ContentResolver;")
    JNI:checkForJNIException(true)

    if not contentResolver then
        return nil
    end

    local getStringMethodId = self.env[0].GetStaticMethodID(
        self.env,
        settingsSecure,
        "getString",
    ...
    JNI:checkForJNIException(true)
    if not getStringMethodId then
        return nil
    end

    local androidID = self.env[0].CallStaticObjectMethod(
        self.env,
        settingsSecure,
        getStringMethodId,
        contentResolver,
        self:to_javastring("android_id")
    )
    JNI:checkForJNIException(true)
    if not androidID then
        return nil
    end

    return self:to_luastring(androidID)
end

function JNI:getResourceID(name, defType, defPackage)
    local resources = self:callObjectMethod(getActivity(), "getResources", "()Landroid/content/res/Resources;")
    local resourceID = self:callIntMethod(resources, "getIdentifier", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)I",
                                          self:to_javastring(name),
                                          self:to_javastring(defType),
                                          self:to_javastring(defPackage))
    return resourceID
end
